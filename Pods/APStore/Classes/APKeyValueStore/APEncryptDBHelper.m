...
@implementation APEncryptDBHelper
...
+ (BOOL)changeKey:(NSString *)dbPath originKey:(NSString *)originKey newKey:(NSString *)newKey
{
    ...
    if (sqlite3_open([dbPath UTF8String], &encrypted_DB) == SQLITE_OK) {
        ...
        sqlite3_exec(encrypted_DB, [[NSString stringWithFormat:@"PRAGMA key = '%@';", originKey] UTF8String], NULL, NULL, &errmsg);
        
        if (errmsg) {
            
        }
        
        sqlite3_exec(encrypted_DB, [[NSString stringWithFormat:@"PRAGMA rekey = '%@';", newKey] UTF8String], NULL, NULL, &errmsg);
